  ...
  <div>
    <v-container>
        ...
        <div class="d-flex">
          ...
          <v-card-text>
            <template v-if="inputMessageId === message.id">
              <v-text-field v-model="newMessageContent" @keyup.enter="showInput"></v-text-field>
            </template>
            <template v-else>
              {{ message.content }}
            </template>
      ...
      <div class="d-flex " style="position: fixed; width: 100%; max-width: inherit; bottom: 0; ">
        <v-text-field v-model="content" @keyup.enter="addMessage" class="mr-2" style="flex-grow: 1;"></v-text-field>
