...
// Utilisez l'instance de Pinia pour accéder à votre store


export default defineWebSocketHandler({
  async open(peer) {
    console.log("[ws] open", peer);
    const forumIdStore = useForumIdStore(pinia);
  
    // Utilisez une méthode getter pour récupérer la valeur
    let forumIdstr = await forumIdStore.getForumId();
    console.log("[ws] forumId", forumIdstr);
    let forumId = parseInt(forumIdstr);
